...
__global__ void kernel_cudaCrop( unsigned char *input, int inputWidth,
    ...
    for ( int c = 0; c < inputChannel; ++c )
        output[ y * outwcSize + x * inputChannel + c ] = ( float )input[ y_pos * inwcSize + x_pos * inputChannel + c ];
}
...
__global__ void kernel_cudaResize_nn( float *input, int inputWidth,
    ...
    for ( int c = 0; c < inputChannel; ++c )
        output[ y * outwcSize + x * inputChannel + c ] = input[ dy * inwcSize + dx * inputChannel + c ];
}
...
__global__ void kernel_cudaResize_bilinear( float *output, const int inputWidth,
    ...
    const float3 d00 = {tex2D( texRef, d0_xpos, ind_y ), tex2D( texRef, d0_xpos + 1, ind_y ), tex2D( texRef, d0_xpos + 2, ind_y )};
    const float3 d10 = {tex2D( texRef, d1_xpos, ind_y ), tex2D( texRef, d1_xpos + 1, ind_y ), tex2D( texRef, d1_xpos + 2, ind_y )};
    const float3 d11 = {tex2D( texRef, d1_xpos, ind_y + 1 ), tex2D( texRef, d1_xpos + 1, ind_y + 1 ), tex2D( texRef, d1_xpos + 2, ind_y + 1 )};
    const float3 d01 = {tex2D( texRef, d0_xpos, ind_y + 1 ), tex2D( texRef, d0_xpos + 1, ind_y + 1 ), tex2D( texRef, d0_xpos + 2, ind_y + 1 )};
...
__global__ void kernel_packed_to_planar( float *input, int inputWidth,
    ...
    for ( int c = 0; c < inputChannel; ++c )
        output[ c * imgSize + y * outputWidth + x ] = input[ y * wcSize + x * inputChannel + c ];
}
...
/* crop image
    ---------
*  input : bgr_packed_uchar_gpu image
*  inputWidth, inputHeight, inputChannel : width, height, channel of input image
*  output : bgr_packed_float_gpu image which size is [outputWidth, outputHeight]
*  xMin, yMin : Left Top offset of crop section
*  outputWidth, outputHeight : width, height of crop(output) image
*/
bool cudaCrop( gpu_shared_ptr<uint8_t> input, int inputWidth, int inputHeight,
    ...
    if ( inputWidth * inputHeight * inputChannel * outputHeight * outputWidth == 0 || xMin < 0 || yMin < 0 )
        return false;
...
/* resize planar_bgr_gpu image
    ---------
*  input : bgr_packed_float_gpu image
*  inputWidth, inputHeight, inputChannel : width, height, channel of input image
*  output : bgr_packed_float_gpu image which size is [outputWidth, outputHeight]
*  interpolation : interpolation method (0 : nearest neighbor, 1 : bilinear)
*  outputWidth, outputHeight : width, height of crop(output) image
*/
bool cudaResize( gpu_shared_ptr<float> input, int inputWidth, int inputHeight,
    ...
    if ( inputWidth * inputHeight * inputChannel * outputWidth * outputHeight == 0 )
        false;

    // 0 : nearest neighbor interpolation / other : bilinear interpolation
    if ( interpolation == 0 ) {
        ...
        const float2 scale = make_float2( inputWidth * 1.0f / outputWidth,
                                          inputHeight * 1.0f / outputHeight );
        const dim3 blockDim( 16, 16 );
        const dim3 gridDim( ( outputWidth + blockDim.x - 1 ) / blockDim.x,
...
*  input : bgr_packed_float_gpu image
*  inputWidth, inputHeight, inputChannel : width, height, channel of input image
*  output : bgr_planar_float_gpu image 
*  outputWidth, outputHeight : width, height of crop(output) image
*/
...
/* merge bgr_planar_float_gpu images 
*  ( example: if batch size is 4 and inputs size is 7, 
    output size is 2 which the one is merged with 4 inputs
    and the other is merged with 3 image and 1 black image)
    ---------
*  inputs : array of bgr_planar_float_gpu images
*  inputWidth, inputHeight, inputChannel : width, height, channel of input image
*  initInputIdx : init_index of input array
*  output : bgr_planar_float_gpu image which merged the number of batch images.
*  outputWidth, outputHeight : width, height of crop(output) image
*  batch : size of batch
*/
bool cudaMerge( std::vector<gpu_shared_ptr<float>> inputs, int inputWidth,
                int inputHeight, int inputChannel, uint8_t initInputIdx,
...
bool cropResizeBGR( std::shared_ptr<cv::Mat> bgr_packed_cpu, std::shared_ptr<SimpleBBoxes> boxes, std::vector<gpu_shared_ptr<float>> bgr_planar_gpu_array, int outputWidth, int outputHeight ) {
    // null check
    if ( bgr_packed_cpu == nullptr || boxes == nullptr || outputWidth * outputHeight == 0 )
        return false;
    ...
    // resize crop images [outputWidth, outputHeight] in GPU
    ...
    //bgr_packed_float_gpu to bgr_planar_float_gpu
    ...
    for ( int j = 0; j < mergeImgSize; j++ ) {
        ...
        const uint8_t init_idx = j * batch_num;
        cudaMerge( d_planarImgs, outputWidth, outputHeight, inputChannel, init_idx, bgr_planar_gpu, batch_num );
